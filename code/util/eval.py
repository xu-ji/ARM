...
def evaluate_basic(config, tasks_model, data_loader, t, is_val, last_classes=None, seen_classes=None,
  ...
  if compute_forgetting_metric:
    ...
    if len(getattr(config, "%s_accs" % prefix)) >= 3: # at least 1 previous (non pre training) eval
      ...
      getattr(config, "%s_forgetting" % prefix)[t] = compute_forgetting(config, t, all_per_label_acc, last_classes, seen_classes)
...
def compute_forgetting(config, t, all_per_label_acc, last_classes, seen_classes):
  ...
  num_post_0_evals_so_far = int(t / config.eval_freq) # t = 1000, freq = 100, just done 10 evals incl this one
