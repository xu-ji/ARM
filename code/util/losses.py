...
def TV(x):
  ...
  h_x = x.shape[2]
  w_x = x.shape[3]
  count_h = batch_sample_size(x[:, :, 1:, :]) # num points in 1 image inc channels except 1 row missing
...
def notlocal(preds, present_classes):
  # maximise distance with present classes
  # due to floating point, plus optimisation starting (i.e. away) from orig samples, don't enforce img difference
...
def neg_symmetric_KL(aux_preds_old, aux_preds_new):
  ...
  return 1 - 0.5 * (F.kl_div(F.log_softmax(aux_preds_old, dim=1), avg_preds, reduction="batchmean") +
