...
def get_cifar10_loaders(config):
  ...
  cifar10_training = cifar10(root=config.data_path, train=True, transform=transform_train,
                             non_stat=(not config.stationary), two_classes_per_block=two_classes_per_block,
                             num_iterations=config.num_iterations)

  if not config.stationary:
    cifar10_training_loader = DataLoader(cifar10_training, sampler=SequentialSampler(cifar10_training), shuffle=False, batch_size=config.tasks_train_batch_sz)
  else:
    cifar10_training_loader = DataLoader(cifar10_training, shuffle=True, batch_size=config.tasks_train_batch_sz)

  test_fns = [transforms.ToTensor()]
  transform_test = transforms.Compose(test_fns)

  evals_non_stat = False # does not make a difference, test time behavior independent of rest of batch
  ...
  cifar10_test = cifar10(root=config.data_path, train=False, transform=transform_test, num_iterations=1,
                          non_stat=evals_non_stat, two_classes_per_block=two_classes_per_block)
  cifar10_test_loader = DataLoader(cifar10_test, shuffle=False, batch_size=config.tasks_eval_batch_sz)
...
def get_miniimagenet_loaders(config):
  ...
  miniimagenet_val = miniimagenetval(root=config.data_path, transform=transform_test,
                                     non_stat=evals_non_stat, classes_per_task=config.classes_per_task)
...
def get_mnist5k_loaders(config):
  ...
  mnist5k_val = mnist5k(root=config.data_path, data_type="val", non_stat=evals_non_stat, num_iterations=1)
  mnist5k_val_loader = DataLoader(mnist5k_val, shuffle=False, batch_size=config.tasks_eval_batch_sz)

  mnist5k_test = mnist5k(root=config.data_path, data_type="test", non_stat=evals_non_stat, num_iterations=1)
  mnist5k_test_loader = DataLoader(mnist5k_test, shuffle=False, batch_size=config.tasks_eval_batch_sz)
