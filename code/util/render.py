...
def render_aux_x(config, t, r, aux_x_orig, aux_x, aux_y, present_classes):
  ...
  if not aux_x is None:
    ...
    if not config.render_separate:
      fig0, axarr0 = plt.subplots(2, max(config.render_aux_x_num, 2), sharex=False, figsize=(config.render_aux_x_num * 4, 2 * 4))
    ...
    for i in range(config.render_aux_x_num):
      ...
      if not config.render_separate:
        ...
        vals = (top_class.item(), s_aux_y[i, top_class].item(), str(list(present_classes.cpu().numpy())), orig_min.item(), orig_max.item())
        ...
        fig_curr_orig.savefig(os.path.join(render_out_dir, "m_%d_t_%d_%r_i_%d_orig.png" % (config.model_ind, t, r, i)), bbox_inches=0)
        ...
        fig_curr_aux.savefig(os.path.join(render_out_dir, "m_%d_t_%d_%r_i_%d_aux_%d_%.3f.png" %
                                          (config.model_ind, t, r, i, top_class.item(), s_aux_y[i, top_class].item())), bbox_inches=0)

        plt.close("all")

    if not config.render_separate:
      fig0.suptitle("Model %d t %d r %d, diffs %f" % (config.model_ind, t, r, diff_sum), fontsize=8)
      fig0.savefig(os.path.join(render_out_dir, "render_m_%d_t_%d_r_%d.png" % (config.model_ind, t, r)))
      plt.close("all")
  else:
